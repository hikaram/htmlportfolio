/**
 * ѕлагин jQuery дл€ создани€ плавающего виджета с кнопками.
 * –ассчитан на использование на страницах с фиксированной шириной.
 * ¬иджет будет по€вл€тьс€ слева или справа от основного содержимого страницы. 
 * 
 * »спользование:
 ...
 * «десь кнопка создаЄтс€ сервисом Tweetmeme (http://tweetmeme.com/about/retweet_button),
 ...
 * fromCenter - рассто€ние от середины страницы до левого кра€ виджета
 * fromTop - рассто€ние от виджета до верхней границы видимой части страницы
 * minTop - рассто€ние от виджета до начала страницы
 * location - размещение виджета (left или right)
 * 
 * »спользуютс€ параметры следующим образом
 * $('#twitter_widget').floating_panel({
 *  		'fromCenter': 520,
 *  		'fromTop': 50,
 *  		'minTop': 200,
...
(function($) {
    $.fn.floating_panel = function(settings) {
    	...
    	//рассчитываем смещение от левого кра€ окна браузера
    	...
    	//измен€ем положени€ виджета при прокрутке страницы 
    	curWindow.scroll(function() {
   			updateElement();
    	});
    	
    	function updateElement() {
    		//рассто€ние от начала страницы до верха еЄ видимой части
